...
export class SaleDetailComponent implements OnInit {
  ...
  async save() {
    ...
    if (this.curQuotation.customerId == 'new') {
      ...
      let filterCustomer = this.$customers.filter(
        item => item.id == this.curQuotation.customerId
      );
      this.curQuotation['customerName'] = filterCustomer[0].company;
    }

    if (this.curQuotation.companyId) {
      let filterCompany = this.$companies.filter(
        item => item.id == this.curQuotation.companyId
      );
  ...
  resetNullValueOfDropdownList(){
    for(let key in this.curQuotation){
      if(this.curQuotation[key] == "null"){
